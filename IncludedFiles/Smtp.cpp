...
BOOL CSMTPAttachment::Attach(const CString& sFilename)
{
  ...
  char* pszIn = new char[(UINT)fs.m_size];
  infile.Read(pszIn, (UINT)fs.m_size);

  //allocate the encoded buffer
  int nOutSize = Base64BufferSize((UINT)fs.m_size);
  m_pszEncoded = new char[nOutSize];

  //Do the encoding
  EncodeBase64(pszIn, (UINT)fs.m_size, m_pszEncoded, nOutSize, &m_nEncodedSize);
  ...
  TCHAR sExt[_MAX_EXT];
  _tsplitpath_s(sFilename, NULL, 0, NULL, 0, sFname, _MAX_FNAME, sExt, _MAX_EXT);
  _tmakepath_s(sPath, _MAX_PATH, NULL, NULL, sFname, sExt);
	m_sFilename = sPath;
  m_sTitle = sPath;
...
void CSMTPMessage::AddBody(const CString& sBody, bool abIsHTML)
{
  m_sBody = sBody;
  m_bIsHTML = abIsHTML;
...
BOOL CSMTPMessage::AddMultipleRecipients(const CString& sRecipients, RECIPIENT_TYPE RecipientType)
{
	...
	_tcscpy_s(buf, length, sRecipients);
...
BOOL CSMTPConnection::Connect(LPCTSTR pszHostName, int nPort, LPCTSTR aszCustomHELOparameter)
{
	//For correct operation of the T2A macro, see MFC Tech Note 59
	USES_CONVERSION;
  ...
  else
  {
	...
	if(aszCustomHELOparameter != NULL && strlen(aszCustomHELOparameter) > 0)
	{
		pszHostName = (LPTSTR)aszCustomHELOparameter;
	}
	else
	{
		//retreive the localhost name
		gethostname(sHostName, sizeof(sHostName));
		pszHostName = A2T(sHostName);
	}
...
BOOL CSMTPConnection::ReadResponse(LPSTR pszBuffer, int nInitialBufSize, LPSTR pszTerminator, int nExpectedCode, LPSTR* ppszOverFlowBuffer, int nGrowBy)
{
  ...
  //determine if the response is an error
	char sCode[4];
	strncpy_s(sCode, sizeof(sCode), pszRecvBuffer, 3);
	sCode[3] = '\0';
